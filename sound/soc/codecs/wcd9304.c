...
extern int afe_loopback_gain(u16 port_id, u16 volume);
extern int get_fm_radio_state(void);
...
struct sitar_priv {
	...
	struct work_struct hs_disable_FM_volume_work;
...
static bool sitar_hs_skip_btn_press(struct sitar_priv *sitar)
{
    bool ret = false;
	...
	{
#define HS_GPIO  50
#define GPIO_LEVEL_INSERT 0

	    if (gpio_get_value_cansleep(HS_GPIO) != GPIO_LEVEL_INSERT)
	    {
	        ret = true;
	    }
	}
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_update_bits(codec, w->reg, 0x0E, 0x00);//songyy 2013.4.19
				     ...
				     int mask)
{
	...
	snd_soc_jack_report_no_dapm(jack, status, mask); // chenjun:orig:snd_soc_jack_report

/* ZTE_Audio_LCX_130325, lichaoxia, 2013-03-25, start */
#if defined(CONFIG_US_EURO_SWITCH)
if (jack == sitar->mbhc_cfg.headset_jack)
{
...
else if (jack == sitar->mbhc_cfg.button_jack)
{
	...
	if (sitar) {
		if (sitar->mbhc_cfg.button_jack) {
			...
			dce_mv = sitar_codec_sta_dce_v(sitar->codec, 1,
							bias_value);
			if ((sitar->in_gpio_handler) || sitar_hs_skip_btn_press(sitar))
			{
...
{
	bool invalid = false;
	// chenjun:struct sitar_priv *sitar = snd_soc_codec_get_drvdata(codec);
...
#else
		    pr_err("chenjun:%s:mic_volt(%d) still valid\n", __func__, mic_volt);
#endif
/* ZTE_Audio_CJ_120530, chenjun, 2012-05-30, end */
#endif
	}
...
pr_debug("chenjun:%s: v_no_mic(%d), v_hs_max(%d)\n", __func__, plug_type_ptr->v_no_mic, plug_type_ptr->v_hs_max);
...
pr_debug("chenjun:%s: i(%d):mic_mv[%d](%d):plug_type[%d](%d):r(%d)\n", __func__, i, i, mic_mv[i], i, plug_type[i], r);
...
/* ZTE_Audio_CJ_120530, chenjun, 2012-05-30, start */
				...
				r = true;
#else
			  if (((PLUG_TYPE_HEADPHONE == plug_type[i-1]) && (PLUG_TYPE_HEADSET == plug_type[i]))
			      || ((PLUG_TYPE_HEADSET == plug_type[i-1]) && (PLUG_TYPE_HEADPHONE == plug_type[i])))
			  {
			    plug_type[0] = PLUG_TYPE_HEADSET;
			    pr_err("chenjun:%s: Detect [%d]%d and [%d]%d, in this case, use type(%d)",
			       __func__, i-1, plug_type[i-1], i, plug_type[i], plug_type[0] );
			  }
			  else
			  {
			    pr_err("chenjun:%s: Detect [%d]%d and [%d]%d are not same, use type(%d)",
			       __func__, i-1, plug_type[i-1], i, plug_type[i], plug_type[0] );
			  }
#endif
/* ZTE_Audio_CJ_120530, chenjun, 2012-05-30, end */
...
pr_debug("chenjun:%s:invalid(%d):plug_type(%d)\n", __func__, r, plug_type[0]);
...
static void tabla_hs_disable_FM_volume(struct work_struct *work)
{
	afe_loopback_gain(INT_FM_TX, 0);
	pr_err("in tabla_hs_disable_FM_volume");
}
...
/* ZTE_Audio_LCX_130325, lichaoxia, 2013-03-25, end */
		...
		sitar_schedule_hs_detect_plug(sitar);
	} else/* chenjun:if (plug_type[0] == PLUG_TYPE_HEADSET) */{
...
/* ZTE_Audio_LCX_130325, lichaoxia, 2013-03-25, end */
		...
		if (is_removed) {
			if (get_fm_radio_state()) {
	schedule_work(&priv->hs_disable_FM_volume_work);
	pr_err("schedule_work hs_disable_FM_volume_work");
	}
			...
			const struct sitar_mbhc_config *cfg)
{
	...
	INIT_WORK(&sitar->hs_disable_FM_volume_work,
			  tabla_hs_disable_FM_volume);
...
static irqreturn_t sitar_dce_handler(int irq, void *data)
{
	...
	if ((priv->in_gpio_handler) || sitar_hs_skip_btn_press(priv)) {
