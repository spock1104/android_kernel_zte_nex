...
static int
led_wled_set(struct pm8xxx_led_data *led, enum led_brightness new_value)
{
	int rc, duty,value;
...
led_mpp_set(struct pm8xxx_led_data *led, enum led_brightness value)
{
	...
	printk("led_mpp_set : %s %d(1-5MA, 2-10MA, 3-15MA, 4-20MA, 5-25MA, 6-30MA, 7-35MA, 8-40MA)\n", led->cdev.name, value);
	if (value==0){
		...
		/*
		value=1 PM8XXX_MPP_CS_OUT_5MA
		value=2 PM8XXX_MPP_CS_OUT_10MA
		value=3 PM8XXX_MPP_CS_OUT_15MA
		value=4 PM8XXX_MPP_CS_OUT_20MA
		value=5 PM8XXX_MPP_CS_OUT_25MA
		value=6 PM8XXX_MPP_CS_OUT_30MA
		value=7 PM8XXX_MPP_CS_OUT_35MA
		value=8 PM8XXX_MPP_CS_OUT_40MA
		*/
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	printk(LED_DRIVER_TAG "set blink state=%d. %s\n",state, led_cdev->name);
...
if (led->pwm_duty_cycles == NULL) {
	...
	printk(LED_DRIVER_TAG "brightness=%d blink=%d duty=%d start=%d len=%d low=%d hi=%d. %s\n",led->cdev.brightness,
...
case PM8XXX_ID_WLED:
	//printk("\n LCD backlight 2 wled value=%d",level);	
...
if (rc) { 
	...
	dev_err(led->dev->parent, "can't write register 0x268 value" 
...
static int __devinit pm8xxx_led_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pcore_data->num_leds; i++) {
		...
		if (rc) {
			printk(LED_DRIVER_TAG "DBG device_create_file blink for %s led failed\n",led_dat->cdev.name);
		}
		...
		if (led_cfg->default_state)
			...
			#if(defined (CONFIG_FB_MSM_MIPI_WARPLTE_OTM_VIDEO_WVGA_PT_PANEL)||defined (CONFIG_MACH_HERA)||defined (CONFIG_MACH_IRIS) || defined (CONFIG_MACH_OCEANUS))
