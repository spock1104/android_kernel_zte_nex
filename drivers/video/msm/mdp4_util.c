...
if (isr & INTR_PRIMARY_VSYNC) {
	...
	int64_t this_fps_update = ktime_to_ms(ktime_get());
	cputime64_t last_diff = cputime64_sub((cputime64_t)this_fps_update,(cputime64_t)last_fps_update);
...
#endif
        if ((last_diff > 40) && (vidc_playing == false)){
                i = snprintf(buf, 64, "fps delay %llu ms\n", last_diff);
		trace_console(buf, 0, i-1, i);
        }
}//tcdtcd
