...
static int mdp_on(struct platform_device *pdev)
{
	...
	if (mfd &&
	    mfd->new_frame_rate &&
	    MIPI_VIDEO_PANEL == mfd->panel_info.type &&
	    mfd->panel_info.mipi.frame_rate != mfd->new_frame_rate) {
		printk(KERN_ERR"rms:(%s:%d)panel_info %u", __FUNCTION__, __LINE__, mfd->new_frame_rate);
		mfd->panel_info.frame_interval = 1000 / mfd->new_frame_rate;
	}
	//end
	if (mdp_rev >= MDP_REV_40) {
		mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
