...
struct pm8921_bms_chip {
	...
	struct wake_lock		get_iavg_wake_lock;	//zte jiangfeng added
					...
					int usb_chg, unsigned int uv)
{
	...
	printk("usb_chg: %d, xoadc_v0625: %d, xoadc_v125: %d, local_delta %d\n",
		usb_chg,chip->xoadc_v0625, chip->xoadc_v0625, local_delta);
				...
				int batt_temp_decidegc)
{
	...
	if (chip->prev_last_good_ocv_raw == OCV_RAW_UNINITIALIZED) {
		...
		printk("%s,PON_OCV_UV = %d\n", __func__,chip->last_ocv_uv);		//zte jiangfeng
...
static int get_battery_uvolts(struct pm8921_bms_chip *chip, int *uvolts)
{
	...
	pr_debug("mvolts phy = %lld meas = 0x%llx", result.physical,
						result.measurement);
	printk("%s,mvolts phy = %lld meas = 0x%llx", __func__,result.physical,
						result.measurement);			//zte jiangfeng
				...
				int *ret_pc_unusable)
{
	...
	pr_debug("For i_ma = %d, unusable_rbatt = %d unusable_uv = %d unusable_pc = %d uuc = %d\n",
					...
					pc_unusable, uuc);
	printk("%s, i_ma = %d, unusable_rbatt = %d unusable_uv = %d unusable_pc = %d uuc = %d\n",
					__func__,i_ma, uuc_rbatt_uv, unusable_uv,
					pc_unusable, uuc);
				...
				int iavg_ua, int delta_time_s)
{
	...
	if (firsttime && chip->shutdown_iavg_ua != 0) {
		...
		printk("%s, Using shutdown_iavg_ua = %d in all samples\n",
				__func__,chip->shutdown_iavg_ua);
...
static void read_shutdown_soc_and_iavg(struct pm8921_bms_chip *chip)
{
	...
	printk("%s,shutdown_soc = %d shutdown_iavg = %d shutdown_soc_invalid = %d\n",
			__func__,
			chip->shutdown_soc,
			chip->shutdown_iavg_ua,
			shutdown_soc_invalid);					//zte jiangfeng
}
...
static void calib_hkadc(struct pm8921_bms_chip *chip)
{
	...
	if (chip->xoadc_v0625_usb_present && chip->xoadc_v0625_usb_absent) {
		...
		printk("625V_present= %d, 625V_absent= %d, delta = %duV\n",
			chip->xoadc_v0625_usb_present,
			chip->xoadc_v0625_usb_absent,
			last_usb_cal_delta_uv);		//zte add
	}
					...
					int batt_temp, int chargecycles)
{
	...
	if (fcc_uah - unusable_charge_uah <= 0) {
		...
		printk("%s,FCC = %duAh, UUC = %duAh forcing soc = 0\n",
						__func__,fcc_uah, unusable_charge_uah);
		soc = 0;
	} else {
	...
	if (firsttime && soc < 0) {
		 ...
		 */
		printk("%s,first time, soc %d, batt temp %d, charge cycles %d, rbatt %d, \
				fcc_uah %d, unusable_charge_uah %d, cc_uah %d\n",
				__func__,soc, batt_temp, chargecycles,
				rbatt, fcc_uah, unusable_charge_uah, cc_uah);		//zte jiangfeng
		...
		printk("DONE for O soc is %d, pon ocv adjusted to %duV\n",
				soc, chip->last_ocv_uv);
#endif
	}
	...
	if (firsttime && soc != shutdown_soc
		...
		printk("DONE for shutdown_soc = %d soc is %d, adjusted ocv to %duV\n",
				shutdown_soc, soc, chip->last_ocv_uv);
	}
	...
	printk("%s,soc %d, batt temp %d, charge cycles %d, rbatt %d, \
		fcc_uah %d, unusable_charge_uah %d, cc_uah %d, remaining uah %d\n",
		__func__,soc, batt_temp, chargecycles,
			rbatt, fcc_uah, unusable_charge_uah, cc_uah, remaining_usable_charge_uah);
...
static int recalculate_soc(struct pm8921_bms_chip *chip)
{
	...
	printk("%s, ocv_raw= 0x%x, ocv_uv= %duV,cc = %d\n",
			__func__,raw.last_good_ocv_raw, raw.last_good_ocv_uv, raw.cc);		//zte jiangfeng
...
static void check_initial_ocv(struct pm8921_bms_chip *chip)
{
	...
	printk("%s,ocv raw = %d, ocv_uv = %d last_ocv_uv = %d\n", __func__,ocv_raw,ocv_uv, chip->last_ocv_uv);	//zte jiangfeng
}
...
#else
extern void* get_bms_battery_data(void);
static int set_battery_data(struct pm8921_bms_chip *chip)
{
	int64_t battery_id;
	struct bms_battery_data* palladium_data;

	palladium_data	=	(struct bms_battery_data*)get_bms_battery_data();
	battery_id = read_battery_id(chip);
	if (battery_id < 0) {
...
//zte jiangfeng add
static int zte_start_get_iave	=	0;
static int zte_average_current	=	0;
static unsigned int zte_second_of_iavg	=	0;
static int zte_last_cc	=	0;
static int zte_cc	=	0;

static void zte_calculate_cc_uah(struct pm8921_bms_chip *chip, int cc, int *val)
{
	int64_t cc_voltage_uv, cc_uvh, cc_uah;
...
static void get_average_current_timer(unsigned long data)
{
	int last_cc_uah, cc_uah;
	int delta;
	int negative	=	0;
	printk("get average current\n");
	
	read_cc(the_chip, &zte_cc);
	zte_calculate_cc_uah(the_chip, zte_last_cc, &last_cc_uah);
	zte_calculate_cc_uah(the_chip, zte_cc, &cc_uah);

	delta	=	cc_uah-last_cc_uah;
	if(delta	<	0)
	{
		delta	=	abs(delta);
		negative	=	1;
	}
	printk("delta: %d, negative: %d\n",delta,negative);
	zte_average_current	=	delta*3600/1000/zte_second_of_iavg;
	printk("delta: %d, negative: %d, average current: %d\n", delta, negative, zte_average_current);
	if(negative)
		zte_average_current	=	-zte_average_current;
	//zte_average_current	=	(cc_uah-last_cc_uah)*3600/1000/zte_second_of_iavg;
	printk("%s, zte_last_cc %d, zte_cc %d, last_cc_uah %d, cc_uah %d, average current %d\n",
		__func__, zte_last_cc, zte_cc, last_cc_uah, cc_uah,zte_average_current);
	
	wake_unlock(&the_chip->get_iavg_wake_lock);
	zte_start_get_iave	=	0;
}
...
//zte jiangfeng add	
	...
	chip->timer.function = get_average_current_timer;
	
	wake_lock_init(&chip->get_iavg_wake_lock, WAKE_LOCK_SUSPEND, "pm8921_get_iavg");
//zte jiangfeng add, end
...
//zte jiangfeng add
void zte_get_average_current(unsigned int second)
{
	if(the_chip)
	{
		//if(zte_start_get_iave)
			del_timer(&the_chip->timer);

		wake_lock(&the_chip->get_iavg_wake_lock);
		read_cc(the_chip, &zte_last_cc);

		the_chip->timer.expires	=	jiffies + second * HZ;
		add_timer(&the_chip->timer);
		zte_start_get_iave	=	1;
	}
}

void zte_stop_get_average_current(void)
{
	del_timer(&the_chip->timer);
}
	
static int set_second_of_iavg_param(const char *val, struct kernel_param *kp)
{
	int ret;
	...
	printk("set second_of_iavg %d",zte_second_of_iavg);
	return 0;
}

static int get_second_of_iavg_param(char *buffer, struct kernel_param *kp)
{
	return	sprintf(buffer,"%d",zte_second_of_iavg);
}

module_param_call(second_of_iavg, set_second_of_iavg_param, get_second_of_iavg_param,
					&zte_second_of_iavg, 0644);

static int zte_start_get_iave_param(const char *val, struct kernel_param *kp)
{
	int ret;
	...
	printk("%s, second %d, start %d\n", __func__,zte_second_of_iavg, zte_start_get_iave);
	if(zte_start_get_iave)
	{
		if(zte_second_of_iavg)
			zte_get_average_current(zte_second_of_iavg);
	}
	else
	{
		zte_stop_get_average_current();
	}
		
	return 0;
}
module_param_call(start_get_iavg, zte_start_get_iave_param, param_get_uint,
						&zte_start_get_iave, 0644);


static int set_average_current_param(const char *val, struct kernel_param *kp)
{
	int ret;
...
module_param_call(average_current, set_average_current_param, param_get_int,
					&zte_average_current, 0644);
//zte jiangfeng add, end
