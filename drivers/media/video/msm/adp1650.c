...
int ADP1650_disable(void)
{
	...
	rc = adp1650_write(adp1650_client, 0x04,0xA4);			
	return rc;
}
int ADP1650_assistmode_enable(void)
{    
	int rc=0;	
	rc = adp1650_write(adp1650_client, 0x04,0xAE);	
	return rc;
}
...
{
	int rc=0;
	rc = adp1650_write(adp1650_client, 0x02,0x07);
	rc = adp1650_write(adp1650_client, 0x03,0x73);//43
	rc = adp1650_write(adp1650_client, 0x04,0xAF);
	rc = gpio_request(18, "adp1650");
...
#else
		ret = gpio_request(64,"FLASH_NAME");
		if (!ret) {
			pr_err("%s: gpio_request1 64 OK\n", __func__);
			...
			ret = gpio_request(64,"FLASH_NAME");
			if (!ret){
			pr_err("%s: gpio_request 64 OK\n", __func__);
			gpio_direction_output(64,1);
			usleep_range(1000, 2000);
			mdelay(1);
			}else{
			pr_err("%s: gpio_request 64 failed\n", __func__);
			}
		}
#endif
