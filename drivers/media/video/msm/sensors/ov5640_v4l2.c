...
uint16_t current_preview_gain_flash=0;
//modify for preview abnormal in mono mode after snapshot  by lijing ZTE_CAM_LJ_20120627]
static int8_t  zte_effect=0;
static int8_t  zte_sat=3;
static int8_t  zte_wb_mode=1;
static int8_t  zte_contrast=3;//-2
static int8_t  zte_sharpness=3;//-2
static int8_t  zte_brightness=3;
#define ZTE_FIX_WB_ENABLE
 #ifdef ZTE_FIX_WB_ENABLE
extern  uint32_t current_flash_led_mode;
static int     zte_disable_wb_auto_flag=0; 
static int     zte_flash_on_fix_wb(struct msm_sensor_ctrl_t *s_ctrl);
static int 	   zte_flash_on_fix_wb_setreg(struct msm_sensor_ctrl_t *s_ctrl);
#endif
...
{0x5583, 0x40}, // increase saturation by neil 0x40
{0x5584, 0x20}, //kenxu @20110706 uvadjust 0x40-->0x18 from 8xgain-->16xgain 0x1e
{0x3814, 0x31},
{0x3815, 0x31},
{0x3803, 0x04},
{0x3807, 0x9b},
...
{0x4004, 0x02},
{0x4005, 0x18},
{0x4837, 0x11},//niujing modify mipi before 0x2a
{0x4713, 0x03},
{0x4407, 0x04},
{0x460b, 0x35},
{0x460c, 0x22},
...
{0x460c, 0x20},
{0x3824, 0x01},
#if 1  //yanwei normal capture use 20130104
{0x3035, 0x21}, //normal capture
#else
{0x3035, 0x11}, //yanwei ZSL use 20130104
#endif
{0x3036, 0x70},//0x70 10fps 0x7e 11fps
#else
{0x3035, 0x21}, // for capture 7.5fps 42M sysclk
{0x3036, 0x70}, //0718 5a-6.4fps  69-7.5fps  70-8fps 7e-9fps
{0x3c07, 0x07}, 
{0x3814, 0x11}, 
{0x3815, 0x11}, 
{0x3803, 0x00}, 
...
{0x3a13, 0x43},
{0x3a18, 0x00},
{0x3a19, 0xf8},//20130425 neil 0x80
{0x3635, 0x13},
{0x3636, 0x03},
{0x3634, 0x40},
{0x3622, 0x01},
...
static struct msm_camera_i2c_conf_array ov5640_init_conf[] = {
	...
	ARRAY_SIZE(ov5640_reset_settings), 10, MSM_CAMERA_I2C_BYTE_DATA},
...
static struct msm_camera_i2c_reg_conf ov5640_lens_shading_D65_tbl[] = {
	#if 0
//OV5640 85% LENC setting 20130312
{0x5800,0x25},
{0x5801,0x13},
{0x5802,0x10},
{0x5803,0x11},
...
int ov5640_get_auto_flash_flag( struct msm_sensor_ctrl_t *s_ctrl, uint8_t* flag)
{
	int rc=0;
	pr_err("%s: E\n", __func__);
	
	rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client,
			0x350b, &current_preview_gain_flash,MSM_CAMERA_I2C_BYTE_DATA);
	pr_err("%s: current_preview_gain_flash=%d\n", __func__,current_preview_gain_flash);	
	if (rc < 0) {
	...
	*flag=ov5640_autoflashflag;
	pr_err("%s: flag=%d\n", __func__,*flag);
	return 0;
}

int ov5640_set_focusmode(struct msm_sensor_ctrl_t *s_ctrl,
		struct msm_sensor_v4l2_ctrl_info_t *ctrl_info, int value)
{   
	int rc = 0;
	uint16_t caf_r1,caf_r2;	  
	pr_err(" %s focusmode==%d  focusmode_state=%d",__func__,value,focusmode_state);  
...
static int32_t ov5640_set_wb(struct msm_sensor_ctrl_t *s_ctrl,
		struct msm_sensor_v4l2_ctrl_info_t *ctrl_info, int wb_val)
{
    long rc = 0;
    pr_err("%s: entry: wb_val=%d\n", __func__, wb_val);
...
#if 0
static int32_t ov5640_set_iso(struct msm_sensor_ctrl_t *s_ctrl,
		struct msm_sensor_v4l2_ctrl_info_t *ctrl_info, int iso_val)
{
    int32_t rc = 0;
	...
	mdelay(100);
		...
		mdelay(100);	
            rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x3A18 ,0x0000, MSM_CAMERA_I2C_BYTE_DATA);//ZTE_YGL_CAM_20110711,modifed for SNR
            if (rc < 0)
            {
               return rc;
            }
            rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x3A19 ,0x00c0, MSM_CAMERA_I2C_BYTE_DATA);//ZTE_YGL_CAM_20110711,modifed for SNR
            if (rc < 0)
            {
               return rc;
            }
		...
		mdelay(100);	
            rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x3A18 ,0x0000, MSM_CAMERA_I2C_BYTE_DATA);//add for iso1600 by yanwei
            if (rc < 0)
            {
               return rc;
            }
            rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x3A19 ,0x00f8, MSM_CAMERA_I2C_BYTE_DATA);//add for iso1600 by yanwei
            if (rc < 0)
            {
               return rc;
            }
...
#endif
static int32_t ov5640_set_saturation(struct msm_sensor_ctrl_t *s_ctrl,
		struct msm_sensor_v4l2_ctrl_info_t *ctrl_info, int saturation_val)
{
    long rc = 0;
    uint16_t tmp_reg = 0;
	...
	rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x5583, 0x0000, MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0)
	{
	return rc;
	}            

	rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x5584, 0x0000, MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0)
	{
	return rc;
	}
...
//	tmp_reg &= 0x00FF;
	...
	*/
	...
	rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x5583, 0x0070, MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0)
	{
	return rc;
	}            

	rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x5584, 0x0058, MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0)
	{
	return rc;
	}
	...
	zte_sat=saturation_val;		
		}
    return rc;
}

static int32_t ov5640_set_contrast(struct msm_sensor_ctrl_t *s_ctrl,
		struct msm_sensor_v4l2_ctrl_info_t *ctrl_info, int contrast_val)
{
    long rc = 0;
    uint16_t tmp_reg = 0;
    ...
    switch(contrast_val)
    {
        ...
        case MSM_V4L2_CONTRAST_L5:
        {
            ...
            rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x5586, 0x0028, MSM_CAMERA_I2C_BYTE_DATA);
            if (rc < 0)
            {
               return rc;
            }            
	...
	zte_contrast=contrast_val;		
		}
    return rc;
}

static int32_t ov5640_set_brightness(struct msm_sensor_ctrl_t *s_ctrl,
		struct msm_sensor_v4l2_ctrl_info_t *ctrl_info, int brightness_val)
{
    long rc = 0;
    uint16_t tmp_reg = 0;
	...
	zte_brightness=brightness_val;
    return rc;
}

static int32_t ov5640_set_exposure_compensation(struct msm_sensor_ctrl_t *s_ctrl,
		struct msm_sensor_v4l2_ctrl_info_t *ctrl_info, int exposure_val)
{
    long rc = 0;
    pr_err("%s: entry: exposure=%d\n", __func__, exposure_val);
    ...
    switch(exposure_val)
    {
        ...
        {
            rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x3a0f, 0x0034, MSM_CAMERA_I2C_BYTE_DATA);
            if (rc < 0)
            {
               return rc;
            }
            rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x3a10, 0x002B, MSM_CAMERA_I2C_BYTE_DATA);
            if (rc < 0)
            {
               return rc;
            }
            ...
            rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x3a1b, 0x0034, MSM_CAMERA_I2C_BYTE_DATA);
            if (rc < 0)
            {
               return rc;
            }
            rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x3a1e, 0x002B, MSM_CAMERA_I2C_BYTE_DATA);
            if (rc < 0)
            {
               return rc;
            }
...
static int32_t ov5640_set_sharpness(struct msm_sensor_ctrl_t *s_ctrl,
		struct msm_sensor_v4l2_ctrl_info_t *ctrl_info, int sharpness_val)
{
    int32_t rc = 0;
    pr_err("%s: entry: sharpness=%d\n", __func__, sharpness_val);
	if (zte_effect == CAMERA_EFFECT_OFF) 
	{       
    switch (sharpness_val)
    {
        case MSM_V4L2_SHARPNESS_L0:
        {
            ...
            rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x5302 ,0x0000, MSM_CAMERA_I2C_BYTE_DATA);
            if (rc < 0)
            {
               return rc;
            }
        ...
        case MSM_V4L2_SHARPNESS_L2:
        {
            ...
            rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x5302 ,0x0017, MSM_CAMERA_I2C_BYTE_DATA);
            if (rc < 0)
            {
               return rc;
            }
        ...
        case MSM_V4L2_SHARPNESS_L4:
        {
            ...
            rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x5302 ,0x0030, MSM_CAMERA_I2C_BYTE_DATA);
            if (rc < 0)
            {
               return rc;
            }
        ...
        case MSM_V4L2_SHARPNESS_L5:
        {
            ...
            rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x5302 ,0x0040, MSM_CAMERA_I2C_BYTE_DATA);
            if (rc < 0)
            {
               return rc;
            }
        ...
        case MSM_V4L2_SHARPNESS_L6:
        {
            ...
            rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x5302 ,0x0050, MSM_CAMERA_I2C_BYTE_DATA);
            if (rc < 0)
            {
               return rc;
            }
		...
		struct msm_sensor_v4l2_ctrl_info_t *ctrl_info, int value)
{
	...
	rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x5583, 0x0040, MSM_CAMERA_I2C_BYTE_DATA); //by neil 0x44
	if (rc < 0)
	{
	return rc;
	}
	rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x5584, 0x001e, MSM_CAMERA_I2C_BYTE_DATA); //by neil 0x18
	if (rc < 0)
	{
	return rc;
	}
...
int set_delay( struct msm_sensor_ctrl_t *s_ctrl,
		struct msm_sensor_v4l2_ctrl_info_t *ctrl_info, int value)
{
	pr_err("%s: value=%d \n", __func__,value);
	msleep(value);	
...
static int32_t  ov5640_set_antibanding( struct msm_sensor_ctrl_t *s_ctrl,
		struct msm_sensor_v4l2_ctrl_info_t *ctrl_info, int antibanding)
{
    int32_t rc = 0;
    int16_t tmp_reg = 0;
...
#if defined(CONFIG_MACH_DEMETER)	
	{
	tmp_reg = 0;
	rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client, 0x3c01, &tmp_reg, MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0)
...
#elif defined(CONFIG_MACH_APOLLO) || defined(CONFIG_MACH_BECKY)  || defined(CONFIG_MACH_METIS) 
        {
            tmp_reg = 0;
            rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client, 0x3c01, &tmp_reg, MSM_CAMERA_I2C_BYTE_DATA);
            if (rc < 0)
...
struct msm_sensor_v4l2_ctrl_info_t ov5640_v4l2_ctrl_info[] = {
	...
	{
		.ctrl_id = V4L2_CID_FOCUSMODE, 
		.min = 0,
		.max = 5,
		.step = 1,
		.s_v4l2_ctrl = ov5640_set_focusmode,
	},
	...
	{
		.ctrl_id = V4L2_CID_GET_PREVIEW_EXPOSURE_GAIN_VALUE,
		.min = 0,
		.max = 1,
		.step = 1,
...
*/
	{
		.ctrl_id = V4L2_CID_DELAY,
		.min = 0,
		.max = 1,
		.step = 1,
		.s_v4l2_ctrl = set_delay,
	},
	#if 0
	{
...
static struct msm_sensor_output_info_t ov5640_dimensions[] = {
	{
		...
		.op_pixel_clk = 168000000,//128000000  224000000
		.binning_factor = 1,
	},
	
	{
		.x_output =  0x500, //0x280-640, //add 0x500-0x3C0= 1280*96p
		.y_output = 0x3C0, //0x1e0-480,  
		.line_length_pclk = 0x500, //280
		.frame_length_lines = 0x3C0, // 1e0
		.vt_pixel_clk = 48000000,
		.op_pixel_clk = 224000000,//128000000 224000000
		.binning_factor = 1,
	},
...
int msm_camera_ov5640_power_on(struct msm_sensor_ctrl_t *s_ctrl)
	{
	int32_t rc = 0;
	...
	if (rc < 0) {
		pr_err("%s: enable moto_ldo failed\n", __func__);
	}else{		
	pr_err("%s  moto_ldo=1\n",__func__);	
		}	

#if defined(CONFIG_MACH_DEMETER)
	gpio_request(43,"ov5640");
	rc=gpio_direction_output(43, 1);
	if (rc < 0) {
		pr_err("%s: enable iovdd_ldo failed\n", __func__);
	}else{		
	pr_err("%s  iovdd_ldo=1\n",__func__);	
		}
#endif
	gpio_request(55,"ov5640");
	...
	gpio_request(9,"ov5640");	
	rc=gpio_direction_output(9, 1);
	if (rc < 0) {
		pr_err("%s: enable dvdd_ldo failed\n", __func__);
	}else{		
	pr_err("%s  dvdd_ldo=1\n",__func__);	
		}	
...
int32_t msm_sensor_ov5640_i2c_probe(struct i2c_client *client,const struct i2c_device_id *id)
	{
	int rc = 0;
	...
	msm_camera_ov5640_power_on(s_ctrl);
	if (rc < 0) {
...
static struct i2c_driver ov5640_i2c_driver = {
	...
	.probe  = msm_sensor_ov5640_i2c_probe,
	.driver = {
	.name = SENSOR_NAME,
	},
};
...
int32_t msm_sensor_ov5640_power_up(struct msm_sensor_ctrl_t *s_ctrl)
	{
	int32_t rc = 0;
	struct device *dev = NULL;
	pr_err("%s  %d \n",__func__,__LINE__);
	...
	gpio_request(54,"ov5640");	
	rc=gpio_direction_output(54, 0);
	if (rc < 0) {
		pr_err("%s: enable PWD failed\n", __func__);
	}else{		
	pr_err("%s  gpio54=1 exit PWD\n",__func__);
		}
	msleep(10);
...
int32_t msm_sensor_ov5640_power_down(struct msm_sensor_ctrl_t *s_ctrl)
	{
	int32_t rc = 0;
	struct device *dev = NULL;	
	pr_err("%s\n", __func__);
	...
	//msm_camera_ov5640_power_off(s_ctrl,s_ctrl->reg_ptr);
	return rc;
	}

static int32_t msm_camera_i2c_write_b_sensor(struct msm_sensor_ctrl_t *s_ctrl,unsigned int waddr, unsigned short bdata)
{
int rc = 0;
...
static int32_t msm_camera_i2c_read_byte_1(struct msm_sensor_ctrl_t *s_ctrl,unsigned short raddr, unsigned short *rdata)
{
int rc = 0;
...
//	int8_t buf[2];
	...
	msm_camera_i2c_read_byte_1(s_ctrl,0x3034, buf);
	buf[1] = buf[0] & 0x0f;
	 if (buf[1] == 8 || buf[1] == 10) {
		 Bit_div2x = buf[1] / 2;
	 }

	msm_camera_i2c_read_byte_1(s_ctrl,0x3035, buf);
	 SysDiv = buf[0] >>4;
	 if(SysDiv == 0) {
		 SysDiv = 16;
	 }

	msm_camera_i2c_read_byte_1(s_ctrl,0x3036, buf);
	 Multiplier = buf[0];

	msm_camera_i2c_read_byte_1(s_ctrl,0x3037, buf);
	 PreDiv = buf[0] & 0x0f;
	 Pll_rdiv = ((buf[0] >> 4) & 0x01) + 1;

	msm_camera_i2c_read_byte_1(s_ctrl,0x3108, buf);
	 buf[1] = buf[0] & 0x03;
...
static int ov5640_get_HTS(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	msm_camera_i2c_read_byte_1(s_ctrl,0x380c, buf);
	msm_camera_i2c_read_byte_1(s_ctrl,0x380d, &buf[1]);
	 HTS=buf[0];
	 HTS = (HTS<<8) + buf[1];
...
static int ov5640_get_VTS(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	msm_camera_i2c_read_byte_1(s_ctrl,0x380e, buf);
	printk("%s: 0x380e=0x%x\n", __func__, buf[0]);
	msm_camera_i2c_read_byte_1(s_ctrl,0x380f, &buf[1]);
	printk("%s: 0x380e=0x%x\n", __func__, buf[1]);
	VTS = buf[0];
	VTS = VTS<<8;
...
static int ov5640_set_VTS(struct msm_sensor_ctrl_t *s_ctrl,int VTS)
{
	unsigned short buf[2];
	 ...
	 msm_camera_i2c_write_b_sensor(s_ctrl,0x380f, buf[0]);

	 buf[0] = VTS>>8;
	 printk("%s: VTS>>8 = 0x%x\n", __func__, buf[0]);
	 msm_camera_i2c_write_b_sensor(s_ctrl,0x380e, buf[0]);
...
static int ov5640_get_shutter(void)
{
	...
	msm_camera_i2c_read_byte_1(s_ctrl,0x3500, buf);
	 shutter = (buf[0] & 0x0f);
	 printk("%s: 0x3500=0x%x\n", __func__, buf[0]);
	 msm_camera_i2c_read_byte_1(s_ctrl,0x3501, buf);
	 shutter = (shutter<<8) + buf[0];
	 printk("%s: 0x3501=0x%x\n", __func__, buf[0]);
	 msm_camera_i2c_read_byte_1(s_ctrl,0x3502, buf);
	 temp = buf[0];
...
static int ov5640_set_shutter(struct msm_sensor_ctrl_t *s_ctrl,int shutter)
{
	...
	unsigned short buf[2];
	 ...
	 msm_camera_i2c_write_b_sensor(s_ctrl,0x3502, buf[0]);

	 temp = shutter & 0xfff;
	 buf[0] = temp>>4;
	 printk("%s: shutter&0xfff >>4 0x%x\n", __func__, buf[0]);
	 msm_camera_i2c_write_b_sensor(s_ctrl,0x3501,buf[0]);

	 temp = shutter>>12;
	 buf[0] = temp;
	 printk("%s: shutter>>12 0x%x\n", __func__, buf[0]);
	 msm_camera_i2c_write_b_sensor(s_ctrl,0x3500, buf[0]);
...
static int ov5640_get_gain16(void)
{
	...
	msm_camera_i2c_read_byte_1(s_ctrl,0x350a, buf);
	 //buf[0] = buf[0] & 0x03;
	 gain16 = buf[0] & 0x03;
	 printk("%s: 0x350a=0x%x\n", __func__, buf[0]);
	 //msm_camera_i2c_read_byte_1(s_ctrl,0x350b, &buf[1], 1, WORD_LEN);
	 msm_camera_i2c_read_byte_1(s_ctrl,0x350b, buf);
...
static int ov5640_set_gain16(struct msm_sensor_ctrl_t *s_ctrl,int gain16)
{
	 ...
	 buf[0] = gain16 & 0xff;
	 msm_camera_i2c_write_b_sensor(s_ctrl,0x350b, buf[0]);

	 buf[0] = gain16>>8;
	 msm_camera_i2c_write_b_sensor(s_ctrl,0x350a, buf[0]);
...
static int ov5640_get_light_frequency(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	msm_camera_i2c_read_byte_1(s_ctrl,0x3c01, buf);

	 if (buf[0] & 0x80) {
		 // manual
		 msm_camera_i2c_read_byte_1(s_ctrl,0x3c00, &buf[1]);
		 if (buf[1] & 0x04) {
			 // 50Hz
			 light_frequency = 50;
		 }
		 ...
		 // auto
		 msm_camera_i2c_read_byte_1(s_ctrl,0x3c0c, &buf[1]);
		 if (buf[1] & 0x01) {
			 // 50Hz
			 light_frequency = 50;
		 }
...
static void ov5640_set_bandingfilter(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	//unsigned short buf[2];
	 ...
	 buf[0] = band_step60 >> 8;
	 msm_camera_i2c_write_b_sensor(s_ctrl,0x3a0a, buf[0]);
	 buf[0] = band_step60 & 0xff;
	 msm_camera_i2c_write_b_sensor(s_ctrl,0x3a0b, buf[0]);

	 max_band60 = (int)((preview_VTS-4)/band_step60);
	 buf[0] = (int8_t)max_band60;
	 msm_camera_i2c_write_b_sensor(s_ctrl,0x3a0d, buf[0]);

	 // 50Hz
	 band_step50 = preview_sysclk * 100/preview_HTS; 
	 buf[0] = (int8_t)(band_step50 >> 8);
	 msm_camera_i2c_write_b_sensor(s_ctrl,0x3a08, buf[0]);
	buf[0] = (int8_t)(band_step50 & 0xff);
	 msm_camera_i2c_write_b_sensor(s_ctrl,0x3a09, buf[0]);

	 max_band50 = (int)((preview_VTS-4)/band_step50);
	 buf[0] = (int8_t)max_band50;
	 msm_camera_i2c_write_b_sensor(s_ctrl,0x3a0e,buf[0]);
}
...
static int zte_flash_on_fix_wb_setreg(struct msm_sensor_ctrl_t *s_ctrl)
{
	int rc = 0;
	pr_err("%s\r\n",__func__);
	rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x3406, 0x01, MSM_CAMERA_I2C_BYTE_DATA); 
	if (rc < 0)
	{
		return rc;
	} 
	...
	rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x3400, 0x07, MSM_CAMERA_I2C_BYTE_DATA); //0x0a
	if (rc < 0)
	{
		return rc;
	} 
   rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x3401, 0x5C, MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0)
	{
		return rc;
	} 
   rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x3402, 0x04, MSM_CAMERA_I2C_BYTE_DATA); //0x05
	if (rc < 0)
	{
		return rc;
	} 
   rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x3403, 0x50, MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0)
	{
		return rc;
	} 
   ...
   rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x3405, 0xC0, MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0)
	{
		return rc;
	} 	
...
static int  zte_flash_on_fix_wb(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	pr_err("%s",__func__);
    rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client, 0x56a1, &bright_val, MSM_CAMERA_I2C_BYTE_DATA);
    if (rc < 0)
    {
        return rc;
    }
	pr_err("0x56a1 g_bright_val =0x%x  current_flash_led_mode=%d zte_wb_mode=%d",
	bright_val,current_flash_led_mode,zte_wb_mode);
	
   if((zte_wb_mode == 1 )&&((current_flash_led_mode == 1)&&(bright_val < 0x10)))
	{
		zte_flash_on_fix_wb_setreg(s_ctrl);
		pr_err("flash auto ");
	}
	
    if((zte_wb_mode == 1 )&&((current_flash_led_mode == 2)&&(bright_val < 0x30)))
	{
		zte_flash_on_fix_wb_setreg(s_ctrl);
		pr_err("flash on ");
	}
...
static int ov5640_get_preview_exposure_gain(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    pr_err("%s",__func__);
  #if 1 //yanwei normal capture use 20130104
   msm_camera_i2c_write_b_sensor(s_ctrl,0x3503, 0x07);  //normal capture use
   msm_camera_i2c_write_b_sensor(s_ctrl,0x3406, 0x01);
  // msm_camera_i2c_write_b_sensor(s_ctrl,0x5196, 0x23); //freeze awb kenxu update @20120516
  if(nightmode!=0){
  msm_camera_i2c_write_b_sensor(s_ctrl,0x3406, 0x01); //disable awb bian shoudong
  }
   #else
   msm_camera_i2c_write_b_sensor(s_ctrl,0x3503, 0x00);  //yanwei zsl  capture use  20130104 
   #endif
   
	//modify for preview abnormal in mono mode after snapshot  by lijing ZTE_CAM_LJ_20120627
	if(CAMERA_EFFECT_OFF == zte_effect) {
	//keep saturation same for preview and capture
	msm_camera_i2c_read_byte_1(s_ctrl,0x558c, &UV);
	msm_camera_i2c_read_byte_1(s_ctrl,0x5588, &temp);
	temp = temp | 0x40;
	msm_camera_i2c_write_b_sensor(s_ctrl,0x5588, temp); //Manual UV
	msm_camera_i2c_write_b_sensor(s_ctrl,0x5583, UV);
	msm_camera_i2c_write_b_sensor(s_ctrl,0x5584, UV);
	pr_err("preview_UV=%d\n", UV);
	}

  if(nightmode!=0){	
	msm_camera_i2c_read_byte_1(s_ctrl,0x3400, buf);
	msm_camera_i2c_read_byte_1(s_ctrl,0x3401, &buf[1]);
	 R_gain=buf[0];
	 R_gain = (R_gain<<8) + buf[1];
	 	printk("%s: R_gain=0x%x\n", __func__, R_gain);
  	}	
  	
#if 0//read out current R/G/B gain 	
  msm_camera_i2c_read_byte_1(s_ctrl,0x3400, buf);
  msm_camera_i2c_read_byte_1(s_ctrl,0x3401, &buf[1]);
  R_gain=buf[0];
  R_gain = (R_gain<<8) + buf[1];
  ...
  msm_camera_i2c_read_byte_1(s_ctrl,0x3402, buf);
  msm_camera_i2c_read_byte_1(s_ctrl,0x3403, &buf[1]);
  G_gain=buf[0];
  G_gain = (G_gain<<8) + buf[1];
  printk("%s: G_gain=0x%x\n", __func__, B_gain);
  
  msm_camera_i2c_read_byte_1(s_ctrl,0x3404, buf);
  msm_camera_i2c_read_byte_1(s_ctrl,0x3405, &buf[1]);
  B_gain=buf[0];
  B_gain = (B_gain<<8) + buf[1];
  printk("%s: B_gain=0x%x\n", __func__, B_gain);
#endif
	 	
	//keep Lenc same for preview and capture
	msm_camera_i2c_read_byte_1(s_ctrl,0x350a, &LencQ_H);
	msm_camera_i2c_read_byte_1(s_ctrl,0x350b, &LencQ_L);
	msm_camera_i2c_write_b_sensor(s_ctrl,0x5054, LencQ_H);
	msm_camera_i2c_write_b_sensor(s_ctrl,0x5055, LencQ_L);	
	msm_camera_i2c_write_b_sensor(s_ctrl,0x504d, 0x02);	//Manual mini Q	

    //get preview exp & gain
    ret_h = ret_m = ret_l = 0;
    ov5640_preview_exposure = 0;
    msm_camera_i2c_read_byte_1(s_ctrl,0x3500, &ret_h);
    msm_camera_i2c_read_byte_1(s_ctrl,0x3501, &ret_m);
    msm_camera_i2c_read_byte_1(s_ctrl,0x3502, &ret_l);
    ...
    msm_camera_i2c_read_byte_1(s_ctrl,0x380e, &ret_h);
    msm_camera_i2c_read_byte_1(s_ctrl,0x380f, &ret_l);
    ov5640_preview_maxlines = (ret_h << 8) + ret_l;
//    printk("Preview_Maxlines=%d\n", ov5640_preview_maxlines);
	msm_camera_i2c_read_byte_1(s_ctrl,0x558c, &ret_h);
   ...
   printk("0x558c=0x%x\n", ret_h);
    ...
    rc=msm_camera_i2c_read(s_ctrl->sensor_i2c_client,0x350a, &ov5640_gain,
			MSM_CAMERA_I2C_WORD_DATA);
    printk("Gain,0x350b=0x%x\n", ov5640_gain);

//ZTE_YGL_CAM_20111221,modifed for SNR
    YAVG = 0;
    msm_camera_i2c_read_byte_1(s_ctrl,0x56A1, &YAVG);
    WB_T = 0;
    msm_camera_i2c_read_byte_1(s_ctrl,0x51d0, &WB_T);
    printk("WB_T=0x%x\n", WB_T);
 // read preview PCLK	 
 preview_sysclk = ov5640_get_sysclk(s_ctrl);	 
 // read preview HTS	 
...
static int ov5640_set_capture_exposure_gain(struct msm_sensor_ctrl_t *s_ctrl)
{
   ...
   //calculate capture exp & gain
	...
	// read capture VTS
	 ...
	 // get average	  
	 msm_camera_i2c_read_byte_1(s_ctrl,0x56a1,&average);	 
	...
	pr_err(" %s: ov5640_lens_shading_D65_tbl\n", __func__);				 
	}
    else if (WB_T == 0X06)
	{
	...
	pr_err(" %s: ov5640_lens_shading_A_tbl\n", __func__);				 
	 }
    //if (current_flash_led_mode == 0)
    	//{
	    if(capture_gain16 > 54) //reach to max gain 16x, change blc to pass SNR test under low light
         {  
	          ...
	          rc = msm_camera_i2c_write_b_sensor(s_ctrl,0x4009, 0x50);
	          if (rc < 0)
	          {
	            return rc;
	          }
      	    ...
      	    printk("capture_shutter=%ld,capture_gain16=%d\n", capture_shutter,capture_gain16);
  if(nightmode!=0){
    	 //R_gain = R_gain * 98/100;
	 buf[0] = R_gain& 0xff;
	 msm_camera_i2c_write_b_sensor(s_ctrl,0x3401, buf[0]);
	 buf[0] = R_gain>>8;
	 msm_camera_i2c_write_b_sensor(s_ctrl,0x3400, buf[0]);
	 printk("%s: R_gain=0x%x\n", __func__, R_gain);
  	}
	 // write capture gain
			...
			int update_type, int res)
{
	...
	if (update_type == MSM_SENSOR_REG_INIT) {
		...
		rc = msm_camera_i2c_read(s_ctrl->sensor_i2c_client,0x4740, &tmp,MSM_CAMERA_I2C_BYTE_DATA);
		if (rc < 0) {
		pr_err("%s: i2c read failed\n", __func__);
		return rc;
		}
	if (tmp != 0x21) {
		rc=msm_camera_i2c_write(s_ctrl->sensor_i2c_client,0x4740, 0x21,MSM_CAMERA_I2C_BYTE_DATA);
		if (rc < 0) {
		pr_err("%s: i2c write failed\n", __func__);
		return rc;
		}
		...
		msm_camera_i2c_read_byte_1(s_ctrl,0x5588, &tmp);
		tmp = tmp & 0xbf;
		msm_camera_i2c_write_b_sensor(s_ctrl,0x5588, tmp); //Auto UV
	...
	//modify for preview abnormal in mono mode after snapshot  by lijing ZTE_CAM_LJ_20120627
		...
		ov5640_set_contrast(s_ctrl,  NULL, zte_contrast);	
		ov5640_set_sharpness(s_ctrl,  NULL, zte_sharpness);		
		ov5640_set_saturation(s_ctrl,  NULL, zte_sat);		
		ov5640_set_brightness(s_ctrl,  NULL, zte_brightness);
#ifdef ZTE_FIX_WB_ENABLE
		if(zte_disable_wb_auto_flag)
		{
		rc = msm_camera_i2c_write(s_ctrl->sensor_i2c_client, 0x3406, 0x0000, MSM_CAMERA_I2C_BYTE_DATA); //denable wb_auto
...
//	.sensor_write_snapshot_exp_gain = ov5640_write_exp_gain,
	...
	.sensor_mode_init = msm_ov5640_sensor_mode_init,//msm_ov5640_sensor_mode_init,msm_sensor_mode_init
	.sensor_get_output_info = msm_sensor_get_output_info,
	.sensor_config = msm_sensor_config,		
	.sensor_power_up = msm_sensor_ov5640_power_up,//msm_sensor_power_up
	.sensor_power_down = msm_sensor_ov5640_power_down,//msm_sensor_power_down
//	.sensor_probe = msm_sensor_probe,
     .sensor_get_flash_flag = ov5640_get_auto_flash_flag,
...
//	.csic_params = &ov5640_csic_params_array[0],
